...
export class CoreStage implements DeploymentStageInterface {
  ...
  private async deployVault(): Promise<VaultContract> {
    ...
    if (vaultAddress) {
      return new this._web3.eth.Contract(Vault.abi, vaultAddress);
    }

    let result = await this._coreWrapper.deployVaultAsync();

    await writeContractToOutputs(this._networkName, 'Vault', result.address);
    return result;
  }
  ...
  private async deployTransferProxy(): Promise<TransferProxyContract> {
    ...
    let result = await this._coreWrapper.deployTransferProxyAsync();

    await writeContractToOutputs(this._networkName, 'TransferProxy', result.address);
    return result;
  }
