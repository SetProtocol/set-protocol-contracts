...
ChaiSetup.configure();
const web3 = getWeb3();
const TimeLockUpgradeV2 = artifacts.require('TimeLockUpgradeV2');
const { SetProtocolTestUtils: SetTestUtils } = setProtocolUtils;
const setTestUtils = new SetTestUtils(web3);
const { expect } = chai;
const blockchain = new Blockchain(web3);

contract('TimeLockUpgradeV2', accounts => {
  const [
    ownerAccount,
    otherAccount,
  ] = accounts;

  let timeLockUpgradeMock: TimeLockUpgradeV2MockContract;

  const coreHelper = new CoreHelper(ownerAccount, ownerAccount);

  before(async () => {
    ABIDecoder.addABI(TimeLockUpgradeV2.abi);
  });

  after(async () => {
    ABIDecoder.removeABI(TimeLockUpgradeV2.abi);
  });

  beforeEach(async () => {
    await blockchain.saveSnapshotAsync();

    timeLockUpgradeMock = await coreHelper.deployTimeLockUpgradeV2MockAsync();
  });
