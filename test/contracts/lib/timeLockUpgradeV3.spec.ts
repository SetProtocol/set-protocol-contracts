...
import { TimeLockUpgradeV3MockContract } from '@utils/contracts';
...
ChaiSetup.configure();
const web3 = getWeb3();
const TimeLockUpgradeV3 = artifacts.require('TimeLockUpgradeV3');
const { SetProtocolTestUtils: SetTestUtils } = setProtocolUtils;
const setTestUtils = new SetTestUtils(web3);
const { expect } = chai;
const blockchain = new Blockchain(web3);

contract('TimeLockUpgradeV3', accounts => {
  const [
    ownerAccount,
    otherAccount,
  ] = accounts;

  let timeLockUpgradeMock: TimeLockUpgradeV3MockContract;

  const coreHelper = new CoreHelper(ownerAccount, ownerAccount);

  before(async () => {
    ABIDecoder.addABI(TimeLockUpgradeV3.abi);
  });

  after(async () => {
    ABIDecoder.removeABI(TimeLockUpgradeV3.abi);
  });

  beforeEach(async () => {
    await blockchain.saveSnapshotAsync();

    timeLockUpgradeMock = await coreHelper.deployTimeLockUpgradeV3MockAsync();
  });
  ...
  describe('#testTimeLockUpgradeV3', async () => {
    const subjectTestUint: BigNumber = ONE;
    let subjectCaller: Address = ownerAccount;

    async function subject(): Promise<string> {
      return timeLockUpgradeMock.testTimeLockUpgradeV3.sendTransactionAsync(
        subjectTestUint,
        { from: subjectCaller },
      );
    }
    ...
    describe('when the timeLockPeriod is positive && a hash is set', async () => {
      ...
      beforeEach(async () => {
        ...
        const txHash = await timeLockUpgradeMock.testTimeLockUpgradeV3.sendTransactionAsync(
          subjectTestUint,
          { from: subjectCaller },
        );
