...
contract('RebalancingToken', accounts => {
  ...
  describe('#burn', async () => {
    ...
    beforeEach(async () => {
      ...
      console.log('whoops');
      const componentAddresses = _.map(components.slice(0, 2), token => token.address);
      const componentUnits = _.map(components.slice(0, 2), () => ether(4)); // Multiple of naturalUnit
      ...
      newRebalancingToken = setToken.address;
    ...
    describe.only('when burn is called from Rebalance state', async () => {
      beforeEach(async () => {
        const auctionLibrary = libraryAccount;
        const curveCoefficient = ether(1);
        const auctionStartPrice = ether(5);
  ...
  describe('#propose', async () => {
    ...
    beforeEach(async () => {
      ...
      const componentAddresses = _.map(components.slice(1, 3), token => token.address);
      componentUnits = _.map(components.slice(1, 3), () => ether(4)); // Multiple of naturalUnit
      setToken = await coreWrapper.createSetTokenAsync(
        core,
        factory.address,
        componentAddresses,
      ...
      subjectRebalancingToken = setToken.address;
    ...
    describe('when propose is called from the Default state', async () => {
      ...
      describe('but it uses a Set Token not recognized by Core', async () => {
        beforeEach(async () => {
          subjectRebalancingToken = components[0].address;
        });
